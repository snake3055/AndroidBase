...
public class CharacterParser {
     ...
     * @return
     */
    private static int parse2Ascii(String chs) {
        int asc = 0;
        try {
     ...
     * @return 单个字拼音
     */
    public static String parseSingle(String single) {
        if (TextUtils.isEmpty(single))
            return "";

        String result = null;
        int ascii = parse2Ascii(single);
     ...
     * @return 整个字符串拼音
     */
    public static String parseAll(String all) {
        ...
        for (int i = 0; i < all.length(); i++) {
            key = all.substring(i, i + 1);
            if (key.getBytes().length >= 2) {
                value = parseSingle(key);
                if (value == null) {
                    value = "unknown";
                }
            } else {
